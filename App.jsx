        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: 24, backdropFilter: "blur(8px)" }}>
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={FINANCIALS.revenue} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              <XAxis dataKey="year" tick={{ fill: "#6B7280", fontSize: 11 }} />
              <YAxis tick={{ fill: "#6B7280", fontSize: 11 }} />
              <Tooltip itemStyle={{ color: "#F9FAFB", fontWeight: 700 }} contentStyle={{ background: "rgba(15, 23, 42, 0.9)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#F9FAFB" }} />
          ...
          <div style={{ position: "relative", flex: 1, minHeight: 400 }}>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                ...
                <Tooltip 
                  itemStyle={{ color: "#F9FAFB", fontWeight: 700 }} contentStyle={{ background: "rgba(15, 23, 42, 0.9)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, color: "#F9FAFB" }} 
          ...
          <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 16 }}>
            ...
            <ResponsiveContainer width="100%" height={120}>
              <LineChart data={[-20,-15,-10,-5,0,5,10,15,20].map(p => {
                ...
                <Tooltip itemStyle={{ color: "#F9FAFB", fontWeight: 700 }} contentStyle={{ background: "#1F2937", border: "none", borderRadius: 6, fontSize: 11, color: "#F9FAFB" }} formatter={v => ["€" + v + "B"]} />
...
const PersonaPage = ({ effectiveBrands = BRANDS }) => {
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: 24 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#9CA3AF", marginBottom: 16 }}>DAHA DETAYLI KONUMLANDIRMALAR (RADAR)</div>
        ...
        {/* LOYALTY LINE CHART */}
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: 24 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#9CA3AF", marginBottom: 16 }}>NPS & SADAKAT GELİŞİMİ (ÇİZGİ GRAFİĞİ)</div>
          <ResponsiveContainer width="100%" height={260}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        {/* POSITIONING SCATTER */}
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: 24 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#9CA3AF", marginBottom: 16 }}>KATEGORİ İÇİ KONUMLANDIRMA (NORMAL SCATTER)</div>
          <ResponsiveContainer width="100%" height={240}>
        ...
        {/* TIMELINE HISTORY */}
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: 24 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#9CA3AF", marginBottom: 16 }}>MARKA GEÇMİŞİ (TIMELINE)</div>
...
const GeneralAnalysisPage = ({ effectiveBrands = BRANDS }) => {
  const [selectedBrand, setSelectedBrand] = useState(effectiveBrands[0].id);
  const brand = effectiveBrands.find(b => b.id === selectedBrand) || effectiveBrands[0];

  const good = [
    `${brand.nps} NPS skoru ile kategorisinde yüksek müşteri güveni tesis edildi.`,
    `%${brand.margin} operasyonel marj oranı ile karlılık hedefleri tutturuldu.`,
    `Pazarda %${brand.marketShare} paya sahip olarak güçlü pozisyon korundu.`
  ];
  
  const stagnant = [
    `Dijital dönüşüm oranları geçen yıla oranla gelişim göstermedi.`,
    `Z kuşağı (Gen-Z) tüketici kazanımı hedeflenen ivmenin altında kaldı.`,
    `${brand.elasticity} puanlık fiyat esnekliği, karlılık esneyebilmesini kısıtlıyor.`
  ];
  
  const badPast = [
    `Geçmişte D2C (doğrudan tüketiciye) kanallarına geç giriş yapılması pazar payı kaybetti.`,
    `Önceki yıllarda net olmayan marka dili, değer teklifini zayıflattı.`,
    `2022'deki tedarik zinciri krizleri hacim büyümesini ciddi oranda kısıtlamıştı.`
  ];
  
  const future = [
    `Gelecek çeyrekte premium segmente odaklanarak marj oranlarını +150bps artırmak.`,
    `Uzun vadeli Müşteri Yaşam Boyu Değerini (CLV) €${brand.clv} üzerine çıkarmak.`,
    `${brand.categoryGrowth > 5 ? "Yüksek büyüme" : "Stabil"} odaklı Aksiyon Planı ve inovasyon yatırımları.`
  ];

  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
      <SectionTitle sub="12 Maddelik Stratejik Analiz ve Evrim Haritası">Genel Analiz</SectionTitle>
      ...
      <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 14, padding: 24, marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {[
            { year: "2018", text: "Zorluklar ve Rekabet: Pazar payı dalgalanmaları ve kimlik arayışı" },
            { year: "2021", text: "Yeniden Yapılanma: Portföy optimizasyonu stratejisi başladı" },
            { year: "2023", text: "Toparlanma & İvme: Kar marjında ilk stabilizasyon sinyalleri" },
            { year: "2024", text: `Güncel Durum: €${brand.revenue}B gelir ve %${brand.usg} USG büyümesi ile güçlü performans.` }
          ].map((item, i) => (
...
export default function App() {
  ...
  const navItems = [
    ...
    { id: "ai",          label: "Genel Analiz",         icon: "⊛", roles: ["cfo","brandManager","strategist","esg"] },
  ...
  const renderPage = () => {
    switch(page) {
      ...
      case "persona": return <PersonaPage effectiveBrands={effectiveBrands} />;
      case "ai": return <GeneralAnalysisPage effectiveBrands={effectiveBrands} />;
